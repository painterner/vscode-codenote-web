...
function $Xb(target, insertIndex, insertArr) {
...
var $T3b = class {
  ...
  register(def) {
    if (this.a[def.scopeName]) {
      ...
      if (!$Ih(existingRegistration.location, def.location)) {
...
var $U3b = "No TM Grammar registered for this language.";
var $V3b = class extends $sd {
...
function $Ge(nameOrOwner, initialValue) {
  let debugNameData;
  if (typeof nameOrOwner === "string") {
    debugNameData = new $Gd(void 0, nameOrOwner, void 0);
  } else {
    debugNameData = new $Gd(nameOrOwner, void 0, void 0);
  }
  return new $He(debugNameData, initialValue, $Kd);
}
...
function $6d(options, computeFn) {
  return new $0d(new $Gd(options.owner, options.debugName, options.debugReferenceFn), computeFn, void 0, void 0, options.onLastObserverRemoved, options.equalsFn ?? $Kd);
}
$ye($6d);
var DerivedState;
...
function $eJ(languageId, state) {
  const tokens = new Uint32Array(2);
  tokens[0] = 0;
  tokens[1] = (languageId << 0 | 0 << 8 | 0 << 11 | 1 << 15 | 2 << 24) >>> 0;
  return new $zF(tokens, state === null ? $cJ : state);
}
...
var $fJ = class {
  ...
  insert(insertIndex, insertCount) {
    ...
    this.a = $Xb(this.a, insertIndex, arr);
  }
};
...
var $6E = class _$6E {
  static fromLineTokens(lineTokens) {
    ...
    for (let i = 0; i < lineTokens.getCount(); i++) {
      tokenInfo.push(new $7E(lineTokens.getEndOffset(i) - lineTokens.getStartOffset(i), lineTokens.getMetadata(i)));
    }
    return _$6E.create(tokenInfo);
  }
  static create(tokenInfo) {
    return new _$6E(tokenInfo);
  }
  constructor(a) {
    this.a = a;
  }
  toLineTokens(lineContent, decoder) {
    return $9E.createFromTextAndMetadata(this.map((r, t) => ({ text: r.substring(lineContent), metadata: t.metadata })), decoder);
  }
  forEach(cb) {
    let lengthSum = 0;
    for (const tokenInfo of this.a) {
      const range = new $4E(lengthSum, lengthSum + tokenInfo.length);
      cb(range, tokenInfo);
  ...
  slice(range) {
    ...
    for (const tokenInfo of this.a) {
      ...
      if (tokenEndEx > range.start) {
        ...
        result.push(new $7E(tokenInfo.length - deltaBefore - deltaAfter, tokenInfo.metadata));
      }
      lengthSum += tokenInfo.length;
    }
    return _$6E.create(result);
  }
};
var $7E = class {
...
var $8E = class {
  ...
  add(length, metadata) {
    this.a.push(new $7E(length, metadata));
  }
  build() {
    return $6E.create(this.a);
  }
};
...
var $9E = class _$9E {
  static createEmpty(lineContent, decoder) {
    const defaultMetadata = _$9E.defaultTokenMetadata;
    const tokens = new Uint32Array(2);
    tokens[0] = lineContent.length;
    tokens[1] = defaultMetadata;
    return new _$9E(tokens, lineContent, decoder);
  }
  static createFromTextAndMetadata(data, decoder) {
    ...
    return new _$9E(new Uint32Array(tokens), fullText, decoder);
  }
  ...
  constructor(tokens, text, decoder) {
    ...
    if (tokensLength !== text.length) {
      $gb(new Error("Token length and text length do not match!"));
    }
    this.a = tokens;
  ...
  getLanguageId(tokenIndex) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    const languageId = $YE.getLanguageId(metadata);
    return this.languageIdCodec.decodeLanguageId(languageId);
  }
  getStandardTokenType(tokenIndex) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    return $YE.getTokenType(metadata);
  }
  getForeground(tokenIndex) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    return $YE.getForeground(metadata);
  }
  getClassName(tokenIndex) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    return $YE.getClassNameFromMetadata(metadata);
  }
  getInlineStyle(tokenIndex, colorMap) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    return $YE.getInlineStyleFromMetadata(metadata, colorMap);
  }
  getPresentation(tokenIndex) {
    const metadata = this.a[(tokenIndex << 1) + 1];
    return $YE.getPresentationFromMetadata(metadata);
  }
  ...
  findTokenIndexAtOffset(offset) {
    return _$9E.findIndexInTokensArray(this.a, offset);
  }
  inflate() {
    return this;
  }
  ...
  */
  withInserted(insertTokens) {
    ...
    return new _$9E(new Uint32Array(newTokens), text, this.languageIdCodec);
  }
  getTokensInRange(range) {
    const builder = new $8E();
    ...
    for (let tokenIndex = startTokenIndex; tokenIndex <= endTokenIndex; tokenIndex++) {
      const tokenRange = new $4E(this.getStartOffset(tokenIndex), this.getEndOffset(tokenIndex));
...
var $$E = new Uint32Array(0).buffer;
var $_E = class _$_E {
  static deleteBeginning(lineTokens, toChIndex) {
    if (lineTokens === null || lineTokens === $$E) {
      return lineTokens;
    }
    return _$_E.delete(lineTokens, 0, toChIndex);
  }
  static deleteEnding(lineTokens, fromChIndex) {
    if (lineTokens === null || lineTokens === $$E) {
      return lineTokens;
    }
    const tokens = $aF(lineTokens);
    const lineTextLength = tokens[tokens.length - 2];
    return _$_E.delete(lineTokens, fromChIndex, lineTextLength);
  }
  static delete(lineTokens, fromChIndex, toChIndex) {
    if (lineTokens === null || lineTokens === $$E || fromChIndex === toChIndex) {
      return lineTokens;
    }
    const tokens = $aF(lineTokens);
    ...
    const fromTokenIndex = $9E.findIndexInTokensArray(tokens, fromChIndex);
  ...
  static append(lineTokens, _otherTokens) {
    ...
    const myTokens = $aF(lineTokens);
    const otherTokens = $aF(_otherTokens);
  ...
  static insert(lineTokens, chIndex, textLength) {
    ...
    const tokens = $aF(lineTokens);
    const tokensCount = tokens.length >>> 1;
    let fromTokenIndex = $9E.findIndexInTokensArray(tokens, chIndex);
    if (fromTokenIndex > 0) {
...
var $dF = class _$dF {
  static deserialize(buff, offset, result) {
    ...
    const startLineNumber = $1i(buff, offset);
    offset += 4;
    const count = $1i(buff, offset);
    offset += 4;
    const tokens = [];
    for (let i = 0; i < count; i++) {
      const byteCount = $1i(buff, offset);
      offset += 4;
      tokens.push(view32.subarray(offset / 4, offset / 4 + byteCount / 4));
      offset += byteCount;
    }
    result.push(new _$dF(startLineNumber, tokens));
    return offset;
  }
  /**
  ...
  getLineRange() {
    return new $bF(this.a, this.a + this.b.length);
  }
  /**
   * @see {@link b}
   */
  ...
  serialize(destination, offset) {
    $2i(destination, this.a, offset);
    offset += 4;
    $2i(destination, this.b.length, offset);
    offset += 4;
    for (let i = 0; i < this.b.length; i++) {
      const lineTokens = this.b[i];
      if (!(lineTokens instanceof Uint32Array)) {
      ...
      $2i(destination, lineTokens.byteLength, offset);
      offset += 4;
  ...
  applyEdit(range, text) {
    const [eolCount, firstLineLength] = $3E(text);
    this.c(range);
    this.d(new $4D(range.startLineNumber, range.startColumn), eolCount, firstLineLength);
  }
  c(range) {
    ...
    if (firstLineIndex === lastLineIndex) {
      this.b[firstLineIndex] = $_E.delete(this.b[firstLineIndex], range.startColumn - 1, range.endColumn - 1);
      return;
    }
    if (firstLineIndex >= 0) {
      this.b[firstLineIndex] = $_E.deleteEnding(this.b[firstLineIndex], range.startColumn - 1);
      if (lastLineIndex < this.b.length) {
        const lastLineTokens = $_E.deleteBeginning(this.b[lastLineIndex], range.endColumn - 1);
        this.b[firstLineIndex] = $_E.append(this.b[firstLineIndex], lastLineTokens);
        this.b.splice(firstLineIndex + 1, lastLineIndex - firstLineIndex);
      } else {
        this.b[firstLineIndex] = $_E.append(this.b[firstLineIndex], null);
        this.b = this.b.slice(0, firstLineIndex + 1);
      }
    } else {
      const deletedBefore = -firstLineIndex;
      this.a -= deletedBefore;
      this.b[lastLineIndex] = $_E.deleteBeginning(this.b[lastLineIndex], range.endColumn - 1);
  ...
  d(position, eolCount, firstLineLength) {
    ...
    if (eolCount === 0) {
      this.b[lineIndex] = $_E.insert(this.b[lineIndex], position.column - 1, firstLineLength);
      return;
    }
    this.b[lineIndex] = $_E.deleteEnding(this.b[lineIndex], position.column - 1);
    this.b[lineIndex] = $_E.insert(this.b[lineIndex], position.column - 1, firstLineLength);
  ...
  e(insertIndex, insertCount) {
    ...
    this.b = $Xb(this.b, insertIndex, lineTokens);
  }
};
...
var $gJ = class {
  static deserialize(buff) {
    ...
    const result = [];
    for (let i = 0; i < count; i++) {
      offset = $dF.deserialize(buff, offset, result);
    }
    return result;
  }
  constructor() {
  ...
  add(lineNumber, lineTokens) {
    ...
    this.a.push(new $dF(lineNumber, [lineTokens]));
  }
  finalize() {
    return this.a;
  }
  ...
  c(destination) {
    let offset = 0;
    $2i(destination, this.a.length, offset);
    offset += 4;
    for (let i = 0; i < this.a.length; i++) {
...
var $kJ = class {
  ...
  setEndState(lineNumber, state) {
    if (!state) {
      throw new $vb("Cannot set null/undefined state");
    }
    this.b.delete(lineNumber);
    const r = this.a.setEndState(lineNumber, state);
    if (r && lineNumber < this.d) {
      this.b.addRange(new $4E(lineNumber + 1, lineNumber + 2));
    }
    return r;
  }
  acceptChange(range, newLineCount) {
    ...
    this.b.addRangeAndResize(new $4E(range.startLineNumber, range.endLineNumberExclusive), newLineCount);
  }
  acceptChanges(changes) {
    for (const c of changes) {
      const [eolCount] = $3E(c.text);
      this.acceptChange(new $bF(c.range.startLineNumber, c.range.endLineNumber + 1), eolCount + 1);
    }
  }
  invalidateEndStateRange(range) {
    this.b.addRange(new $4E(range.startLineNumber, range.endLineNumberExclusive));
  }
  ...
  getFirstInvalidLine(initialState) {
    ...
    if (!startState) {
      throw new $vb("Start state must be defined");
    }
...
var $mJ = class {
  ...
  removeMin() {
    ...
    if (range.start + 1 === range.endExclusive) {
      ...
      this.a[0] = new $4E(range.start + 1, range.endExclusive);
    }
    return range.start;
  }
  delete(value) {
    ...
    if (idx !== -1) {
      ...
      if (range.start === value) {
        if (range.endExclusive === value + 1) {
          ...
          this.a[idx] = new $4E(value + 1, range.endExclusive);
        }
      } else {
        if (range.endExclusive === value + 1) {
          this.a[idx] = new $4E(range.start, value);
        } else {
          this.a.splice(idx, 1, new $4E(range.start, value), new $4E(value + 1, range.endExclusive));
        }
      }
    }
  }
  addRange(range) {
    $4E.addRange(range, this.a);
  }
  addRangeAndResize(range, newLength) {
    ...
    if (idxFirstMightBeIntersecting === idxFirstIsAfter) {
      const newRange = new $4E(range.start, range.start + newLength);
      if (!newRange.isEmpty) {
      ...
      const newRange = new $4E(start, endEx + delta);
      if (!newRange.isEmpty) {
...
var $R3b = class extends $sd {
  ...
  tokenizeEncoded(line, hasEOL, state) {
    ...
    const sw = shouldMeasure ? new $4e(true) : void 0;
    ...
    if (textMateResult.stoppedEarly) {
      ...
      return new $zF(textMateResult.tokens, state);
    }
    if (this.g) {
...
var $W3b = class extends $FN {
  ...
  retokenize(startLineNumber, endLineNumberExclusive) {
    if (this.m) {
      this.m.store.invalidateEndStateRange(new $bF(startLineNumber, endLineNumberExclusive));
      this.q.schedule();
    }
  }
  async v() {
    ...
    if (r.grammar) {
      const tokenizationSupport = new $S3b(this.u, new $R3b(r.grammar, r.initialState, false, void 0, () => false, (timeMs, lineLength, isRandomSample) => {
        this.s.reportTokenizationTime(timeMs, languageId, r.sourceExtensionId, lineLength, isRandomSample);
      }, false), $sd.None, this.o);
      this.m = new $hJ(this.b.length, tokenizationSupport);
    } else {
      this.m = null;
    }
    this.w();
  ...
  async w() {
    ...
    if (!this.p) {
      const { diffStateStacksRefEq } = await $qF("vscode-textmate", "release/main.js");
      this.p = diffStateStacksRefEq;
    }
    const startTime = (/* @__PURE__ */ new Date()).getTime();
    while (true) {
      let tokenizedLines = 0;
      const tokenBuilder = new $gJ();
      ...
      while (true) {
        ...
        $9E.convertToEndOffset(r.tokens, text.length);
...
var $X3b = class _$X3b {
  ...
  static getChannel(workerServer) {
    return workerServer.getChannel(_$X3b.CHANNEL_NAME);
  }
  static setChannel(workerClient, obj) {
    workerClient.setChannel(_$X3b.CHANNEL_NAME, obj);
  }
};

// out-build/vs/workbench/services/textMate/browser/backgroundTokenization/worker/textMateTokenizationWorker.worker.js
function $Y3b(workerServer) {
  return new $Z3b(workerServer);
}
var $Z3b = class {
  constructor(workerServer) {
    ...
    this.a = $X3b.getChannel(workerServer);
  }
  ...
  async f(grammarDefinitions, onigurumaWASMUri) {
    const vscodeTextmate = await $qF("vscode-textmate", "release/main.js");
    const vscodeOniguruma = await $qF("vscode-oniguruma", "release/main.js");
  ...
  $acceptNewModel(data) {
    ...
    const that = this;
    this.b.set(data.controllerId, new $W3b(uri, data.lines, data.EOL, data.versionId, {
