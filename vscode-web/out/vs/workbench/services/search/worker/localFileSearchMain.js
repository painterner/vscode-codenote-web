  ...
  const resultExpression = function(path, base, hasSibling) {
    ...
    for (let i = 0, n = parsedPatterns.length; i < n; i++) {
      ...
      if (parsedPattern.requiresSiblings && hasSibling) {
        ...
        if (!name) {
          name = base.substr(0, base.length - $tc(path).length);
        }
      }
...
`;
      ...
      const fileRange = new $5D(startLine, matchStartIndex - lineRanges[startLine].start, endLine, matchStartIndex + matchedText.length - lineRanges[endLine].start);
      const previewRange = new $5D(0, matchStartIndex - lineRanges[startLine].start - offset, endLine - startLine, matchStartIndex + matchedText.length - lineRanges[endLine].start - (endLine === startLine ? offset : 0));
      const match = {
        rangeLocations: [{
          source: fileRange,
...
var $BFb = class {
  ...
  d(lines, dirPath, trimForExclusions2) {
    ...
    return $$l(includeExpression, { trimForExclusions: trimForExclusions2 });
  }
...
var $pKc = class {
  ...
  async $listDirectory(handle, query, folderQuery, ignorePathCasing, queryId) {
    ...
    const extUri = new $Eh(() => ignorePathCasing);
  ...
  async h(handle, queryProps, folderQuery, extUri, onFile, token) {
    const folderExcludes = folderQuery.excludePattern?.map((excludePattern) => $$l(excludePattern.pattern ?? {}, { trimForExclusions: true }));
    ...
    const processDirectory = async (directory, prior, ignoreFile) => {
      if (!folderQuery.disregardIgnoreFiles) {
        ...
        await Promise.all(ignoreFiles.map(async (file) => {
          ...
          ignoreFile = new $BFb(ignoreContents, prior, ignoreFile);
        }));
      }
...
function createSearchRegExp(options) {
  return $hg(options.pattern, !!options.isRegExp, {
...
function pathIncludedInQuery(queryProps, path, extUri) {
  if (queryProps.excludePattern && $0l(queryProps.excludePattern, path)) {
    return false;
  }
  if (queryProps.includePattern || queryProps.usingSearchPaths) {
    if (queryProps.includePattern && $0l(queryProps.includePattern, path)) {
      return true;
    }
    if (queryProps.usingSearchPaths) {
      return !!queryProps.folderQueries && queryProps.folderQueries.some((fq) => {
        ...
        if (extUri.isEqualOrParent(uri, searchPath)) {
          const relPath = $qc(searchPath.path, uri.path);
