...
var $plc = class _$plc {
  ...
  static createPatterns(workspaceFolder) {
    ...
    if ($l && workspaceFolder.scheme === Schemas.file) {
      workspaceFolderVariants.push($_g(workspaceFolderPath));
    }
    for (const workspaceFolderVariant of workspaceFolderVariants) {
      ...
      patterns.push(new RegExp($$f(workspaceFolderVariant) + `(${pathPattern}) on line ((\\d+)(, column (\\d+))?)`, "gi"));
      patterns.push(new RegExp($$f(workspaceFolderVariant) + `(${pathPattern}):line ((\\d+)(, column (\\d+))?)`, "gi"));
      patterns.push(new RegExp($$f(workspaceFolderVariant) + `(${pathPattern})(\\s?\\((\\d+)(,(\\d+))?)\\)`, "gi"));
      patterns.push(new RegExp($$f(workspaceFolderVariant) + `(${strictPathPattern})(:(\\d+))?(:(\\d+))?`, "gi"));
    }
    return patterns;
  }
  /**
  ...
  static detectLinks(line, lineIndex, patterns, resourceCreator) {
    ...
    patterns.forEach((pattern) => {
      ...
      while ((match = pattern.exec(line)) !== null) {
        const folderRelativePath = $eg(match[1], ".").replace(/\\/g, "/");
        let resourceString;
        try {
        ...
        if (match[3]) {
          ...
          if (match[5]) {
            ...
            resourceString = $7f("{0}#{1},{2}", resourceString, lineNumber, columnNumber);
          } else {
            resourceString = $7f("{0}#{1}", resourceString, lineNumber);
          }
        }
        const fullMatch = $eg(match[0], ".");
        ...
        if (links.some((link) => $5D.areIntersectingOrTouching(link.range, linkRange))) {
          return;
        }
        links.push({
          range: linkRange,
