...
function $hg(searchString, isRegex, options = {}) {
  if (!searchString) {
...
function $3D(firstString, secondString) {
...
function $XI(tree, z) {
  ...
  if (x.parent.left === x) {
    ...
    if (newSizeLeft !== x.parent.size_left || newLFLeft !== x.parent.lf_left) {
      ...
      $ZI(tree, x.parent, delta, lf_delta);
    }
  }
  $1I(tree, x.parent);
  if (yWasRed) {
...
function $ZI(tree, x, delta, lineFeedCntDelta) {
...
var $Cv = {
  ...
  Locale(tag, options) {
    try {
      ...
      return new Intl.Locale($j, options);
    }
  }
};
...
function $RE(wordSeparators, intlSegmenterLocales) {
  const key = `${wordSeparators}/${intlSegmenterLocales.join(",")}`;
  let result = wordClassifierCache.get(key);
  if (!result) {
    result = new $QE(wordSeparators, intlSegmenterLocales);
  ...
  n(searchData, searcher, text, lineNumber, deltaOffset, resultLen, result, captureMatches, limitResultCount) {
    ...
    if (!captureMatches && searchData.simpleSearch) {
      ...
      while ((lastMatchIndex = text.indexOf(searchString, lastMatchIndex + searchStringLen)) !== -1) {
        if (!wordSeparators || $6I(wordSeparators, text, textLength, lastMatchIndex, searchStringLen)) {
...
function $a7b(modifiedCells, originalCells) {
  const cache = {
...
function computeNumberOfEdits(modified, original) {
  ...
  return $3D(modified.getValue(), original.getValue());
}
...
function $5Rb(originalModel, modifiedModel, diffResult) {
...
var MirrorCell = class {
  ...
  constructor(handle, uri, source, d, versionId, language, cellKind, outputs, metadata, internalMetadata) {
    ...
    this.a = new $Jab(uri, source, d, versionId);
  }
  onEvents(e) {
    this.a.onEvents(e);
    this.b = void 0;
  ...
  f() {
    let hashValue = $hj(104579, 0);
    hashValue = $gj(this.language, hashValue);
    hashValue = $gj(this.getValue(), hashValue);
    hashValue = $gj(this.metadata, hashValue);
    hashValue = $gj(this.internalMetadata?.internalId || "", hashValue);
    for (const op of this.outputs) {
      hashValue = $gj(op.metadata, hashValue);
      for (const output of op.outputs) {
        hashValue = $gj(output.mime, hashValue);
      }
    }
    const digests = this.outputs.flatMap((op) => op.outputs.map((o) => $fj(Array.from(o.data.buffer))));
    for (const digest of digests) {
      hashValue = $hj(digest, hashValue);
    }
    return hashValue;
  }
};
  ...
  static createWithCellId(cells, includeCellContents) {
    const hashValue = cells.map((c) => {
      if (includeCellContents) {
        return `${$gj(c.internalMetadata?.internalId, $hj(104579, 0))}#${c.getComparisonValue()}`;
      } else {
        return `${$gj(c.internalMetadata?.internalId, $hj(104579, 0))}}`;
      }
    });
    return new _CellSequence(hashValue);
  }
...
var $b7b = class {
  ...
  async $computeDiff(originalUrl, modifiedUrl) {
    ...
    const originalMetadata = $Cw(original.metadata, (key) => !original.transientDocumentMetadata[key]);
    const modifiedMetadata = $Cw(modified.metadata, (key) => !modified.transientDocumentMetadata[key]);
    const metadataChanged = JSON.stringify(originalMetadata) !== JSON.stringify(modifiedMetadata);
    const originalDiff = new $2D(CellSequence.create(original), CellSequence.create(modified)).ComputeDiff(false);
    ...
    const cellMapping = $5Rb(originalModel, modifiedModel, { cellsDiff: { changes: originalDiff.changes, quitEarly: false }, metadataChanged: false }).cellDiffInfo;
    ...
    if (!diffUsingCellIds) {
      const result = $a7b(modified.cells, original.cells);
    ...
    const cellsInsertedOrDeletedDiff = new $2D(CellSequence.createWithCellId(original.cells), CellSequence.createWithCellId(modified.cells)).ComputeDiff(false);
    const cellDiffInfo = $5Rb(originalModel, modifiedModel, { cellsDiff: { changes: cellsInsertedOrDeletedDiff.changes, quitEarly: false }, metadataChanged: false }).cellDiffInfo;
      ...
      if (!change.originalLength && change.modifiedLength) {
        ...
        cellDiffInfo.slice(processedIndex, changeIndex).forEach((c) => {
          if (c.type === "unchanged" || c.type === "modified") {
            ...
            if (changed) {
              changes.push(new $YD(c.originalCellIndex, 1, c.modifiedCellIndex, 1));
            }
          }
        });
        changes.push(change);
  ...
  updateCellIdsBasedOnMappings(mappings, originalCells, modifiedCells) {
    ...
    originalCells.map((cell, index) => {
      ...
      cell.internalMetadata.internalId = `${PREFIX_FOR_UNMATCHED_ORIGINAL_CELLS}${$6s()}`;
      const found = mappings.find((r) => r.original === index);
      if (found) {
        cell.internalMetadata.internalId = $6s();
        uuids.set(found.modified, cell.internalMetadata.internalId);
      }
    });
    modifiedCells.map((cell, index) => {
      cell.internalMetadata = cell.internalMetadata || { internalId: "" };
      cell.internalMetadata.internalId = uuids.get(index) ?? $6s();
    });
    return true;
  }
  $canPromptRecommendation(modelUrl) {
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      const searchParams = new $2I("import\\s*pandas|from\\s*pandas", true, false, null);
      ...
      const range = new $5D(1, 1, maxLineCount, textBuffer.getLineLength(maxLineCount) + 1);
