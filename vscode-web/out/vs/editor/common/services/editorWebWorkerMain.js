...
function $3(array, predicate) {
  const idx = $4(array, predicate);
...
function $5(array, predicate) {
  const idx = $6(array, predicate);
  return idx === array.length ? void 0 : array[idx];
}
function $6(array, predicate, startIdx = 0, endIdxEx = array.length) {
  let i = startIdx;
  let j = endIdxEx;
  while (i < j) {
    const k = Math.floor((i + j) / 2);
...
function* $Eb(items, shouldBeGrouped) {
  let currentGroup;
  let last;
...
var $0e = class {
  constructor(options) {
    this.z = 0;
    this.f = options;
    this.g = _globalLeakWarningThreshold > 0 || this.f?.leakWarningThreshold ? new LeakageMonitor(options?.onListenerError ?? $gb, this.f?.leakWarningThreshold ?? _globalLeakWarningThreshold) : void 0;
  ...
  k(requestMessage) {
    ...
    result.then((r) => {
      ...
      if (e.detail instanceof Error) {
        e.detail = $ib(e.detail);
      }
      this.o(new ReplyMessage(this.a, req, void 0, $ib(e)));
    });
  }
  l(msg) {
    const req = msg.req;
...
function propertyIsEvent(name) {
  return name[0] === "o" && name[1] === "n" && $xg(name.charCodeAt(2));
}
function propertyIsDynamicEvent(name) {
  return /^onDynamic/.test(name) && $xg(name.charCodeAt(9));
}
var $W4 = class {
...
function $16(factory) {
  ...
  initialized = true;
  const webWorkerServer = new $W4((msg) => globalThis.postMessage(msg), (workerServer) => factory(workerServer));
...
function $1D(original, modified, pretty) {
  return new $2D(new $ZD(original), new $ZD(modified)).ComputeDiff(pretty).changes;
}
var Debug = class {
...
var DiffChangeHelper = class {
  ...
  MarkNextChange() {
    if (this.d > 0 || this.e > 0) {
      this.a.push(new $YD(this.b, this.d, this.c, this.e));
    }
    this.d = 0;
    this.e = 0;
    this.b = 1073741824;
...
var $2D = class _$2D {
  ...
  constructor(originalSequence, modifiedSequence, continueProcessingPredicate = null) {
    ...
    const [originalStringElements, originalElementsOrHash, originalHasStrings] = _$2D.p(originalSequence);
    const [modifiedStringElements, modifiedElementsOrHash, modifiedHasStrings] = _$2D.p(modifiedSequence);
  ...
  r(originalIndex, newIndex) {
    ...
    const originalElement = _$2D.s(this.b, originalIndex);
    const modifiedElement = _$2D.s(this.c, newIndex);
  ...
  x(originalStart, originalEnd, modifiedStart, modifiedEnd, quitEarlyArr) {
    ...
    if (originalStart > originalEnd || modifiedStart > modifiedEnd) {
      ...
      if (modifiedStart <= modifiedEnd) {
        ...
        changes = [
          new $YD(originalStart, 0, modifiedStart, modifiedEnd - modifiedStart + 1)
        ];
      } else if (originalStart <= originalEnd) {
        Debug.Assert(modifiedStart === modifiedEnd + 1, "modifiedStart should only be one more than modifiedEnd");
        changes = [
          new $YD(originalStart, originalEnd - originalStart + 1, modifiedStart, 0)
        ];
      } else {
        Debug.Assert(originalStart === originalEnd + 1, "originalStart should only be one more than originalEnd");
        Debug.Assert(modifiedStart === modifiedEnd + 1, "modifiedStart should only be one more than modifiedEnd");
    ...
    if (result !== null) {
      ...
      if (!quitEarlyArr[0]) {
        ...
        rightChanges = [
          new $YD(midOriginal + 1, originalEnd - (midOriginal + 1) + 1, midModified + 1, modifiedEnd - (midModified + 1) + 1)
        ];
      }
      return this.I(leftChanges, rightChanges);
    }
    return [
      new $YD(originalStart, originalEnd - originalStart + 1, modifiedStart, modifiedEnd - modifiedStart + 1)
    ];
  }
  y(diagonalForwardBase, diagonalForwardStart, diagonalForwardEnd, diagonalForwardOffset, diagonalReverseBase, diagonalReverseStart, diagonalReverseEnd, diagonalReverseOffset, forwardPoints, reversePoints, originalIndex, originalEnd, midOriginalArr, modifiedIndex, modifiedEnd, midModifiedArr, deltaIsEven, quitEarlyArr) {
    ...
    if (quitEarlyArr[0]) {
      ...
      reverseChanges = [
        new $YD(originalStartPoint, originalEnd - originalStartPoint + 1, modifiedStartPoint, modifiedEnd - modifiedStartPoint + 1)
      ];
    } else {
  ...
  J(left, right, mergedChangeArr) {
    ...
    if (left.originalStart + left.originalLength >= right.originalStart || left.modifiedStart + left.modifiedLength >= right.modifiedStart) {
      ...
      mergedChangeArr[0] = new $YD(originalStart, originalLength, modifiedStart, modifiedLength);
      return true;
    } else {
...
function $If(firstPart, secondPart) {
...
var $gab = class {
  ...
  static chord(firstPart, secondPart) {
    return $If(firstPart, secondPart);
  }
};
function $hab() {
  return {
...
function $6I(wordSeparators, text, textLength, matchStartIndex, matchLength) {
...
var $bab = class {
  static computeUnicodeHighlights(model, options, range) {
    ...
    const searcher = new $7I(null, regex);
    const ranges = [];
    let hasMore = false;
    let m;
    ...
    forLoop: for (let lineNumber = startLine, lineCount = endLine; lineNumber <= lineCount; lineNumber++) {
      ...
      do {
        ...
        if (m) {
          ...
          let word = $iE(startIndex + 1, $fE, lineContent, 0);
          ...
          if (highlightReason !== 0) {
            ...
            ranges.push(new $5D(lineNumber, startIndex + 1, lineNumber, endIndex + 1));
          }
        }
      } while (m);
    }
  ...
  static computeUnicodeHighlightReason(char, options) {
    ...
    switch (reason) {
      ...
      case 3: {
        ...
        const notAmbiguousInLocales = $9g.getLocales().filter((l) => !$9g.getInstance(/* @__PURE__ */ new Set([...options.allowedLocales, l])).isAmbiguous(codePoint));
...
function buildRegExpCharClassExpr(codePoints, flags) {
  const src = `[${$$f(codePoints.map((i) => String.fromCodePoint(i)).join(""))}]`;
  return src;
}
...
var CodePointHighlighter = class {
  constructor(b) {
    ...
    this.ambiguousCharacters = $9g.getInstance(new Set(b.allowedLocales));
  }
  ...
  shouldHighlightNonBasicASCII(character, wordContext) {
    ...
    if (wordContext) {
      for (const char of wordContext) {
        ...
        const isBasicASCII = $Ng(char);
        hasBasicASCIICharacters = hasBasicASCIICharacters || isBasicASCII;
        if (!isBasicASCII && !this.ambiguousCharacters.isAmbiguous(codePoint2) && !$0g.isInvisibleCharacter(codePoint2)) {
    ...
    if (this.b.invisibleCharacters) {
      if (!isAllowedInvisibleCharacter(character) && $0g.isInvisibleCharacter(codePoint)) {
        return 2;
      }
    }
    if (this.b.ambiguousCharacters) {
...
var $zN = class _$zN {
  ...
  flip() {
    return new _$zN(this.lineRangeMapping.flip(), this.changes.map((c) => c.flip()));
  }
};
...
var $4E = class _$4E {
  static fromTo(start, endExclusive) {
    return new _$4E(start, endExclusive);
  }
  static addRange(range, sortedRanges) {
    ...
    if (i === j) {
      ...
      sortedRanges.splice(i, j - i, new _$4E(start, end));
    }
  }
  ...
  static ofLength(length) {
    return new _$4E(0, length);
  }
  static ofStartAndLength(start, length) {
    return new _$4E(start, start + length);
  }
  static emptyAt(offset) {
    return new _$4E(offset, offset);
  }
  constructor(start, endExclusive) {
    this.start = start;
    this.endExclusive = endExclusive;
    if (start > endExclusive) {
      throw new $vb(`Invalid range: ${this.toString()}`);
    }
  }
  get isEmpty() {
  ...
  delta(offset) {
    return new _$4E(this.start + offset, this.endExclusive + offset);
  }
  deltaStart(offset) {
    return new _$4E(this.start + offset, this.endExclusive);
  }
  deltaEnd(offset) {
    return new _$4E(this.start, this.endExclusive + offset);
  }
  get length() {
   ...
   */
  join(other) {
    return new _$4E(Math.min(this.start, other.start), Math.max(this.endExclusive, other.endExclusive));
  }
  /**
  ...
  clip(value) {
    if (this.isEmpty) {
      throw new $vb(`Invalid clipping range: ${this.toString()}`);
    }
...
var $bF = class _$bF {
  static fromRange(range) {
    return new _$bF(range.startLineNumber, range.endLineNumber);
  }
  static fromRangeInclusive(range) {
    return new _$bF(range.startLineNumber, range.endLineNumber + 1);
  }
  static subtract(a, b) {
    if (!b) {
      return [a];
    ...
    if (a.startLineNumber < b.startLineNumber && b.endLineNumberExclusive < a.endLineNumberExclusive) {
      return [
        new _$bF(a.startLineNumber, b.startLineNumber),
        new _$bF(b.endLineNumberExclusive, a.endLineNumberExclusive)
      ];
    } else if (b.startLineNumber <= a.startLineNumber && a.endLineNumberExclusive <= b.endLineNumberExclusive) {
      return [];
    } else if (b.endLineNumberExclusive < a.endLineNumberExclusive) {
      return [new _$bF(Math.max(b.endLineNumberExclusive, a.startLineNumber), a.endLineNumberExclusive)];
    } else {
      return [new _$bF(a.startLineNumber, Math.min(b.startLineNumber, a.endLineNumberExclusive))];
    }
  }
  /**
  ...
  static joinMany(lineRanges) {
    ...
    let result = new $cF(lineRanges[0].slice());
    for (let i = 1; i < lineRanges.length; i++) {
      result = result.getUnion(new $cF(lineRanges[i].slice()));
    }
    return result.ranges;
  }
  static join(lineRanges) {
    if (lineRanges.length === 0) {
      throw new $vb("lineRanges cannot be empty");
    }
    ...
    return new _$bF(startLineNumber, endLineNumberExclusive);
  }
  static ofLength(startLineNumber, length) {
    return new _$bF(startLineNumber, startLineNumber + length);
  }
  /**
   * @internal
   */
  static deserialize(lineRange) {
    return new _$bF(lineRange[0], lineRange[1]);
  }
  constructor(startLineNumber, endLineNumberExclusive) {
    if (startLineNumber > endLineNumberExclusive) {
      throw new $vb(`startLineNumber ${startLineNumber} cannot be after endLineNumberExclusive ${endLineNumberExclusive}`);
    }
    this.startLineNumber = startLineNumber;
   ...
   */
  delta(offset) {
    return new _$bF(this.startLineNumber + offset, this.endLineNumberExclusive + offset);
  }
  deltaLength(offset) {
    return new _$bF(this.startLineNumber, this.endLineNumberExclusive + offset);
  }
  /**
   ...
   */
  join(other) {
    return new _$bF(Math.min(this.startLineNumber, other.startLineNumber), Math.max(this.endLineNumberExclusive, other.endLineNumberExclusive));
  }
  toString() {
    return `[${this.startLineNumber},${this.endLineNumberExclusive})`;
  }
  ...
  toInclusiveRange() {
    ...
    return new $5D(this.startLineNumber, 1, this.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER);
  }
  /**
   * @deprecated Using this function is discouraged because it might lead to bugs: The end position is not guaranteed to be a valid position!
  */
  toExclusiveRange() {
    return new $5D(this.startLineNumber, 1, this.endLineNumberExclusive, 1);
  }
  mapToLineArray(f) {
    const result = [];
   ...
   */
  toOffsetRange() {
    return new $4E(this.startLineNumber - 1, this.endLineNumberExclusive - 1);
  }
  ...
  addMargin(marginTop, marginBottom) {
    return new _$bF(this.startLineNumber - marginTop, this.endLineNumberExclusive + marginBottom);
  }
};
var $cF = class _$cF {
  constructor(c = []) {
  ...
  addRange(range) {
    ...
    const joinRangeStartIdx = $6(this.c, (r) => r.endLineNumberExclusive >= range.startLineNumber);
    const joinRangeEndIdxExclusive = $4(this.c, (r) => r.startLineNumber <= range.endLineNumberExclusive) + 1;
  ...
  contains(lineNumber) {
    const rangeThatStartsBeforeEnd = $3(this.c, (r) => r.startLineNumber <= lineNumber);
    return !!rangeThatStartsBeforeEnd && rangeThatStartsBeforeEnd.endLineNumberExclusive > lineNumber;
  }
  intersects(range) {
    const rangeThatStartsBeforeEnd = $3(this.c, (r) => r.startLineNumber < range.endLineNumberExclusive);
    ...
    while (i1 < this.c.length || i2 < other.c.length) {
      ...
      if (current === null) {
        ...
        if (current.endLineNumberExclusive >= next.startLineNumber) {
          current = new $bF(current.startLineNumber, Math.max(current.endLineNumberExclusive, next.endLineNumberExclusive));
        } else {
          result.push(current);
          current = next;
        }
  ...
  subtractFrom(range) {
    ...
    for (let i = joinRangeStartIdx; i < joinRangeEndIdxExclusive; i++) {
      ...
      if (r.startLineNumber > startLineNumber) {
        result.push(new $bF(startLineNumber, r.startLineNumber));
      }
      startLineNumber = r.endLineNumberExclusive;
    }
    if (startLineNumber < range.endLineNumberExclusive) {
      result.push(new $bF(startLineNumber, range.endLineNumberExclusive));
    }
    return new _$cF(result);
  }
  toString() {
  ...
  getWithDelta(value) {
    return new _$cF(this.c.map((r) => r.delta(value)));
  }
};
...
var $ZF = class _$ZF {
  ...
  static lengthDiffNonNegative(start, end) {
    ...
    if (start.lineCount === end.lineCount) {
      return new _$ZF(0, end.columnCount - start.columnCount);
    } else {
      return new _$ZF(end.lineCount - start.lineCount, end.columnCount);
    }
  }
  static betweenPositions(position1, position2) {
    if (position1.lineNumber === position2.lineNumber) {
      return new _$ZF(0, position2.column - position1.column);
    } else {
      return new _$ZF(position2.lineNumber - position1.lineNumber, position2.column - 1);
    }
  }
  static fromPosition(pos) {
    return new _$ZF(pos.lineNumber - 1, pos.column - 1);
  }
  static ofRange(range) {
    return _$ZF.betweenPositions(range.getStartPosition(), range.getEndPosition());
  }
  static ofText(text) {
    let line = 0;
    let column = 0;
  ...
  add(other) {
    if (other.lineCount === 0) {
      return new _$ZF(this.lineCount, this.columnCount + other.columnCount);
    } else {
      return new _$ZF(this.lineCount + other.lineCount, other.columnCount);
    }
  }
  createRange(startPosition) {
    if (this.lineCount === 0) {
      return new $5D(startPosition.lineNumber, startPosition.column, startPosition.lineNumber, startPosition.column + this.columnCount);
    } else {
      return new $5D(startPosition.lineNumber, startPosition.column, startPosition.lineNumber + this.lineCount, this.columnCount + 1);
    }
  }
  toRange() {
    return new $5D(1, 1, this.lineCount + 1, this.columnCount + 1);
  }
  toLineRange() {
    return $bF.ofLength(1, this.lineCount + 1);
  }
  addToPosition(position) {
    if (this.lineCount === 0) {
      return new $4D(position.lineNumber, position.column + this.columnCount);
    } else {
      return new $4D(position.lineNumber + this.lineCount, this.columnCount + 1);
    }
  }
  addToRange(range) {
    return $5D.fromPositions(this.addToPosition(range.getStartPosition()), this.addToPosition(range.getEndPosition()));
  }
  toString() {
...
var $lN = class {
  ...
  c(position) {
    ...
    if (position.lineNumber > lineCount) {
      ...
      return new $4D(lineCount, lineLength2 + 1);
    }
    if (position.column < 1) {
      return new $4D(position.lineNumber, 1);
    }
    const lineLength = this.getLineLength(position.lineNumber);
    if (position.column - 1 > lineLength) {
      return new $4D(position.lineNumber, lineLength + 1);
    }
    return position;
  }
  getOffsetRange(range) {
    return new $4E(this.getOffset(range.getStartPosition()), this.getOffset(range.getEndPosition()));
  }
  getPosition(offset) {
    const idx = $4(this.a, (i) => i <= offset);
    const lineNumber = idx + 1;
    const column = offset - this.a[idx] + 1;
    return new $4D(lineNumber, column);
  }
  getRange(offsetRange) {
    return $5D.fromPositions(this.getPosition(offsetRange.start), this.getPosition(offsetRange.endExclusive));
  }
  getTextLength(offsetRange) {
    return $ZF.ofRange(this.getRange(offsetRange));
  }
  get textLength() {
    const lineIdx = this.a.length - 1;
    return new $ZF(lineIdx, this.text.length - this.a[lineIdx]);
  }
...
var $mN = class _$mN {
  static fromOffsetEdit(edit, initialState) {
    const edits = edit.edits.map((e) => new $nN(initialState.getTransformer().getRange(e.replaceRange), e.newText));
    return new _$mN(edits);
  }
  static single(originalRange, newText) {
    return new _$mN([new $nN(originalRange, newText)]);
  }
  static insert(position, newText) {
    return new _$mN([new $nN($5D.fromPositions(position, position), newText)]);
  }
  constructor(edits) {
    this.edits = edits;
    $Uc(() => $Vc(edits, (a, b) => a.range.getEndPosition().isBeforeOrEqual(b.range.getStartPosition())));
  }
  /**
  ...
  normalize() {
    ...
    for (const edit of this.edits) {
      if (edits.length > 0 && edits[edits.length - 1].range.getEndPosition().equals(edit.range.getStartPosition())) {
        ...
        edits[edits.length - 1] = new $nN(last.range.plusRange(edit.range), last.text + edit.text);
  ...
  mapPosition(position) {
    ...
    for (const edit of this.edits) {
      ...
      const len = $ZF.ofText(edit.text);
      if (position.isBefore(end)) {
        const startPos = new $4D(start.lineNumber + lineDelta, start.column + (start.lineNumber + lineDelta === curLine ? columnDeltaInCurLine : 0));
    ...
    return new $4D(position.lineNumber + lineDelta, position.column + (position.lineNumber + lineDelta === curLine ? columnDeltaInCurLine : 0));
  }
  mapRange(range) {
    function getStart(p) {
      return p instanceof $4D ? p : p.getStartPosition();
    }
    function getEnd(p) {
      return p instanceof $4D ? p : p.getEndPosition();
    }
  ...
  applyToString(str) {
    const strText = new $rN(str);
    return this.apply(strText);
  }
  inverse(doc) {
    const ranges = this.getNewRanges();
    return new _$mN(this.edits.map((e, idx) => new $nN(ranges[idx], doc.getValueOfRange(e.range))));
  }
  getNewRanges() {
    const newRanges = [];
    ...
    let columnOffset = 0;
    for (const edit of this.edits) {
      const textLength = $ZF.ofText(edit.text);
      const newRangeStart = $4D.lift({
  ...
  toSingle(text) {
    ...
    for (let i = 0; i < this.edits.length; i++) {
      ...
      if (i < this.edits.length - 1) {
        ...
        const gapRange = $5D.fromPositions(curEdit.range.getEndPosition(), nextEdit.range.getStartPosition());
        const gapText = text.getValueOfRange(gapRange);
        newText += gapText;
      }
    }
    return new $nN($5D.fromPositions(startPos, endPos), newText);
  }
  equals(other) {
    return $yb(this.edits, other.edits, (a, b) => a.equals(b));
  }
};
var $nN = class _$nN {
  static joinEdits(edits, initialValue) {
    ...
    return new _$nN($5D.fromPositions(startPos, endPos), newText);
  }
  ...
  extendToCoverRange(range, initialValue) {
    ...
    const textBefore = initialValue.getValueOfRange($5D.fromPositions(newRange.getStartPosition(), this.range.getStartPosition()));
    const textAfter = initialValue.getValueOfRange($5D.fromPositions(this.range.getEndPosition(), newRange.getEndPosition()));
    const newText = textBefore + this.text + textAfter;
    return new _$nN(newRange, newText);
  }
  extendToFullLine(initialValue) {
    const newRange = new $5D(this.range.startLineNumber, 1, this.range.endLineNumber, initialValue.getTransformer().getLineLength(this.range.endLineNumber) + 1);
  ...
  removeCommonPrefix(text) {
    ...
    const commonPrefixLen = $Ag(normalizedOriginalText, normalizedModifiedText);
    const start = $ZF.ofText(normalizedOriginalText.substring(0, commonPrefixLen)).addToPosition(this.range.getStartPosition());
    const newText = normalizedModifiedText.substring(commonPrefixLen);
    const range = $5D.fromPositions(start, this.range.getEndPosition());
    return new _$nN(range, newText);
  }
  isEffectiveDeletion(text) {
    let newText = this.text.replaceAll("\r\n", "\n");
    let existingText = text.getValueOfRange(this.range).replaceAll("\r\n", "\n");
    const l = $Ag(newText, existingText);
    newText = newText.substring(l);
    existingText = existingText.substring(l);
    const r = $Bg(newText, existingText);
...
function rangeFromPositions(start, end) {
  if (start.lineNumber === end.lineNumber && start.column === Number.MAX_SAFE_INTEGER) {
    return $5D.fromPositions(end, end);
  } else if (!start.isBeforeOrEqual(end)) {
    throw new $vb("start must be before end");
  }
  return new $5D(start.lineNumber, start.column, end.lineNumber, end.column);
}
var $oN = class {
  constructor() {
    this.c = void 0;
  ...
  get endPositionExclusive() {
    return this.length.addToPosition(new $4D(1, 1));
  }
  get lineRange() {
  ...
  getLineLength(lineNumber) {
    return this.getValueOfRange(new $5D(lineNumber, 1, lineNumber, Number.MAX_SAFE_INTEGER)).length;
  }
  getTransformer() {
    if (!this.c) {
      this.c = new $lN(this.getValue());
    }
    return this.c;
  }
  getLineAt(lineNumber) {
    return this.getValueOfRange(new $5D(lineNumber, 1, lineNumber, Number.MAX_SAFE_INTEGER));
  }
  getLines() {
...
var $pN = class extends $oN {
  ...
  getValueOfRange(range) {
    if (range.startLineNumber === range.endLineNumber) {
      return this.d(range.startLineNumber).substring(range.startColumn - 1, range.endColumn - 1);
    }
    let result = this.d(range.startLineNumber).substring(range.startColumn - 1);
    for (let i = range.startLineNumber + 1; i < range.endLineNumber; i++) {
      result += "\n" + this.d(i);
    }
    result += "\n" + this.d(range.endLineNumber).substring(0, range.endColumn - 1);
    return result;
  }
  getLineLength(lineNumber) {
    return this.d(lineNumber).length;
  }
  get length() {
    const lastLine = this.d(this.f);
    return new $ZF(this.f - 1, lastLine.length);
  }
};
...
var $rN = class extends $oN {
  constructor(value) {
    super();
    this.value = value;
    this.d = new $lN(this.value);
  }
  getValueOfRange(range) {
    return this.d.getOffsetRange(range).substring(this.value);
  }
  get length() {
    return this.d.textLength;
  }
};
...
var $sN = class _$sN {
  static inverse(mapping, originalLineCount, modifiedLineCount) {
    ...
    for (const m of mapping) {
      const r2 = new _$sN(new $bF(lastOriginalEndLineNumber, m.original.startLineNumber), new $bF(lastModifiedEndLineNumber, m.modified.startLineNumber));
    ...
    const r = new _$sN(new $bF(lastOriginalEndLineNumber, originalLineCount + 1), new $bF(lastModifiedEndLineNumber, modifiedLineCount + 1));
  ...
  static clip(mapping, originalRange, modifiedRange) {
    ...
    for (const m of mapping) {
      ...
      if (original && !original.isEmpty && modified && !modified.isEmpty) {
        result.push(new _$sN(original, modified));
      }
    }
    return result;
  }
  ...
  flip() {
    return new _$sN(this.modified, this.original);
  }
  join(other) {
    return new _$sN(this.original.join(other.original), this.modified.join(other.modified));
  }
  ...
  */
  toRangeMapping() {
    ...
    if (origInclusiveRange && modInclusiveRange) {
      return new $uN(origInclusiveRange, modInclusiveRange);
    } else if (this.original.startLineNumber === 1 || this.modified.startLineNumber === 1) {
      if (!(this.modified.startLineNumber === 1 && this.original.startLineNumber === 1)) {
        throw new $vb("not a valid diff");
      }
      return new $uN(new $5D(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new $5D(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
    } else {
      return new $uN(new $5D(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), new $5D(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER));
    }
  }
  /**
   * This method assumes that the LineRangeMapping describes a valid diff!
  ...
  */
  toRangeMapping2(original, modified) {
    ...
    if (!this.original.isEmpty && !this.modified.isEmpty) {
      return new $uN($5D.fromPositions(new $4D(this.original.startLineNumber, 1), normalizePosition(new $4D(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), original)), $5D.fromPositions(new $4D(this.modified.startLineNumber, 1), normalizePosition(new $4D(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), modified)));
    }
    if (this.original.startLineNumber > 1 && this.modified.startLineNumber > 1) {
      return new $uN($5D.fromPositions(normalizePosition(new $4D(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER), original), normalizePosition(new $4D(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), original)), $5D.fromPositions(normalizePosition(new $4D(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER), modified), normalizePosition(new $4D(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), modified)));
    }
    throw new $vb();
  }
};
function normalizePosition(position, content) {
  ...
  if (position.lineNumber > content.length) {
    return new $4D(content.length, content[content.length - 1].length + 1);
  }
  const line = content[position.lineNumber - 1];
  if (position.column > line.length + 1) {
    return new $4D(position.lineNumber, line.length + 1);
  }
  return position;
}
...
var $tN = class _$tN extends $sN {
  static fromRangeMappings(rangeMappings) {
    const originalRange = $bF.join(rangeMappings.map((r) => $bF.fromRangeInclusive(r.originalRange)));
    const modifiedRange = $bF.join(rangeMappings.map((r) => $bF.fromRangeInclusive(r.modifiedRange)));
    return new _$tN(originalRange, modifiedRange, rangeMappings);
  }
  ...
  flip() {
    return new _$tN(this.modified, this.original, this.innerChanges?.map((c) => c.flip()));
  }
  withInnerChangesFromLineRanges() {
    return new _$tN(this.original, this.modified, [this.toRangeMapping()]);
  }
};
var $uN = class _$uN {
  static fromEdit(edit) {
    const newRanges = edit.getNewRanges();
    const result = edit.edits.map((e, idx) => new _$uN(e.range, newRanges[idx]));
    return result;
  }
  static fromEditJoin(edit) {
    const newRanges = edit.getNewRanges();
    const result = edit.edits.map((e, idx) => new _$uN(e.range, newRanges[idx]));
    return _$uN.join(result);
  }
  static join(rangeMappings) {
    if (rangeMappings.length === 0) {
      throw new $vb("Cannot join an empty list of range mappings");
    }
  ...
  static assertSorted(rangeMappings) {
    for (let i = 1; i < rangeMappings.length; i++) {
      ...
      if (!(previous.originalRange.getEndPosition().isBeforeOrEqual(current.originalRange.getStartPosition()) && previous.modifiedRange.getEndPosition().isBeforeOrEqual(current.modifiedRange.getStartPosition()))) {
        throw new $vb("Range mappings must be sorted");
      }
    }
  }
  constructor(originalRange, modifiedRange) {
  ...
  flip() {
    return new _$uN(this.modifiedRange, this.originalRange);
  }
  /**
  ...
  */
  toTextEdit(modified) {
    ...
    return new $nN(this.originalRange, newText);
  }
  join(other) {
    return new _$uN(this.originalRange.plusRange(other.originalRange), this.modifiedRange.plusRange(other.modifiedRange));
  }
};
function $vN(alignments, originalLines, modifiedLines, dontAssertStartLine = false) {
  const changes = [];
  for (const g of $Eb(alignments.map((a) => $wN(a, originalLines, modifiedLines)), (a1, a2) => a1.original.overlapOrTouch(a2.original) || a1.modified.overlapOrTouch(a2.modified))) {
    const first = g[0];
    const last = g[g.length - 1];
    changes.push(new $tN(first.original.join(last.original), first.modified.join(last.modified), g.map((a) => a.innerChanges[0])));
  }
  $Uc(() => {
    ...
    return $Vc(changes, (m1, m2) => m2.original.startLineNumber - m1.original.endLineNumberExclusive === m2.modified.startLineNumber - m1.modified.endLineNumberExclusive && // There has to be an unchanged line in between (otherwise both diffs should have been joined)
    m1.original.endLineNumberExclusive < m2.original.startLineNumber && m1.modified.endLineNumberExclusive < m2.modified.startLineNumber);
  });
  return changes;
}
function $wN(rangeMapping, originalLines, modifiedLines) {
  ...
  const originalLineRange = new $bF(rangeMapping.originalRange.startLineNumber + lineStartDelta, rangeMapping.originalRange.endLineNumber + 1 + lineEndDelta);
  const modifiedLineRange = new $bF(rangeMapping.modifiedRange.startLineNumber + lineStartDelta, rangeMapping.modifiedRange.endLineNumber + 1 + lineEndDelta);
  return new $tN(originalLineRange, modifiedLineRange, [rangeMapping]);
}
...
var $AN = class {
  computeDiff(originalLines, modifiedLines, options) {
    const diffComputer = new $BN(originalLines, modifiedLines, {
    ...
    for (const c of result.changes) {
      ...
      if (c.originalEndLineNumber === 0) {
        originalRange = new $bF(c.originalStartLineNumber + 1, c.originalStartLineNumber + 1);
      } else {
        originalRange = new $bF(c.originalStartLineNumber, c.originalEndLineNumber + 1);
      }
      let modifiedRange;
      if (c.modifiedEndLineNumber === 0) {
        modifiedRange = new $bF(c.modifiedStartLineNumber + 1, c.modifiedStartLineNumber + 1);
      } else {
        modifiedRange = new $bF(c.modifiedStartLineNumber, c.modifiedEndLineNumber + 1);
      }
      let change = new $tN(originalRange, modifiedRange, c.charChanges?.map((c2) => new $uN(new $5D(c2.originalStartLineNumber, c2.originalStartColumn, c2.originalEndLineNumber, c2.originalEndColumn), new $5D(c2.modifiedStartLineNumber, c2.modifiedStartColumn, c2.modifiedEndLineNumber, c2.modifiedEndColumn))));
      if (lastChange) {
        if (lastChange.modified.endLineNumberExclusive === change.modified.startLineNumber || lastChange.original.endLineNumberExclusive === change.original.startLineNumber) {
          change = new $tN(lastChange.original.join(change.original), lastChange.modified.join(change.modified), lastChange.innerChanges && change.innerChanges ? lastChange.innerChanges.concat(change.innerChanges) : void 0);
          changes.pop();
        }
      }
      changes.push(change);
    ...
    return new $yN(changes, [], result.quitEarly);
  }
};
function computeDiff(originalSequence, modifiedSequence, continueProcessingPredicate, pretty) {
  const diffAlgo = new $2D(originalSequence, modifiedSequence, continueProcessingPredicate);
...
var $iab = class _$iab {
  static trivial(seq1, seq2) {
    return new _$iab([new $jab($4E.ofLength(seq1.length), $4E.ofLength(seq2.length))], false);
  }
  static trivialTimedOut(seq1, seq2) {
    return new _$iab([new $jab($4E.ofLength(seq1.length), $4E.ofLength(seq2.length))], true);
  }
...
var $jab = class _$jab {
  static invert(sequenceDiffs, doc1Length) {
    const result = [];
    $Fb(sequenceDiffs, (a, b) => {
      result.push(_$jab.fromOffsetPairs(a ? a.getEndExclusives() : $kab.zero, b ? b.getStarts() : new $kab(doc1Length, (a ? a.seq2Range.endExclusive - a.seq1Range.endExclusive : 0) + doc1Length)));
    });
    return result;
  }
  static fromOffsetPairs(start, endExclusive) {
    return new _$jab(new $4E(start.offset1, endExclusive.offset1), new $4E(start.offset2, endExclusive.offset2));
  }
    ...
    for (const cur of sequenceDiffs) {
      if (last) {
        if (!(last.seq1Range.endExclusive <= cur.seq1Range.start && last.seq2Range.endExclusive <= cur.seq2Range.start)) {
          throw new $vb("Sequence diffs must be sorted");
        }
      }
      last = cur;
    }
  ...
  swap() {
    return new _$jab(this.seq2Range, this.seq1Range);
  }
  toString() {
    return `${this.seq1Range} <-> ${this.seq2Range}`;
  }
  join(other) {
    return new _$jab(this.seq1Range.join(other.seq1Range), this.seq2Range.join(other.seq2Range));
  }
  delta(offset) {
    if (offset === 0) {
      return this;
    }
    return new _$jab(this.seq1Range.delta(offset), this.seq2Range.delta(offset));
  }
  deltaStart(offset) {
    if (offset === 0) {
      return this;
    }
    return new _$jab(this.seq1Range.deltaStart(offset), this.seq2Range.deltaStart(offset));
  }
  deltaEnd(offset) {
    if (offset === 0) {
      return this;
    }
    return new _$jab(this.seq1Range.deltaEnd(offset), this.seq2Range.deltaEnd(offset));
  }
  intersectsOrTouches(other) {
  ...
  intersect(other) {
    ...
    return new _$jab(i1, i2);
  }
  getStarts() {
    return new $kab(this.seq1Range.start, this.seq2Range.start);
  }
  getEndExclusives() {
    return new $kab(this.seq1Range.endExclusive, this.seq2Range.endExclusive);
  }
};
var $kab = class _$kab {
  static {
    this.zero = new _$kab(0, 0);
  }
  static {
    this.max = new _$kab(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
  }
  ...
  delta(offset) {
    ...
    return new _$kab(this.offset1 + offset, this.offset2 + offset);
  }
  equals(other) {
...
var $mab = class {
  constructor(e) {
    ...
    this.d = true;
    if (e <= 0) {
      throw new $vb("timeout must be positive");
    }
  }
...
var $qab = class {
  compute(sequence1, sequence2, timeout = $lab.instance, equalityScore) {
    if (sequence1.length === 0 || sequence2.length === 0) {
      return $iab.trivial(sequence1, sequence2);
    }
    const lcsLengths = new $nab(sequence1.length, sequence2.length);
    const directions = new $nab(sequence1.length, sequence2.length);
    const lengths = new $nab(sequence1.length, sequence2.length);
    for (let s12 = 0; s12 < sequence1.length; s12++) {
      for (let s22 = 0; s22 < sequence2.length; s22++) {
        if (!timeout.isValid()) {
          return $iab.trivialTimedOut(sequence1, sequence2);
        }
    ...
    function reportDecreasingAligningPositions(s12, s22) {
      if (s12 + 1 !== lastAligningPosS1 || s22 + 1 !== lastAligningPosS2) {
        result.push(new $jab(new $4E(s12 + 1, lastAligningPosS1), new $4E(s22 + 1, lastAligningPosS2)));
      }
...
var $rab = class {
  compute(seq1, seq2, timeout = $lab.instance) {
    ...
    loop: while (true) {
      d++;
      if (!timeout.isValid()) {
        return $iab.trivialTimedOut(seqX, seqY);
      }
    ...
    while (true) {
      ...
      if (endX !== lastAligningPosS1 || endY !== lastAligningPosS2) {
        result.push(new $jab(new $4E(endX, lastAligningPosS1), new $4E(endY, lastAligningPosS2)));
      }
      if (!path) {
        break;
      }
...
var $sab = class {
  ...
  get text() {
    return this.getText(new $4E(0, this.length));
  }
  getText(range) {
  ...
  translateOffset(offset, preference = "right") {
    const i = $4(this.c, (value) => value <= offset);
    const lineOffset = offset - this.c[i];
    return new $4D(this.g.startLineNumber + i, 1 + this.d[i] + lineOffset + (lineOffset === 0 && preference === "left" ? 0 : this.f[i]));
  }
  translateRange(range) {
    const pos1 = this.translateOffset(range.start, "right");
    const pos2 = this.translateOffset(range.endExclusive, "left");
    if (pos2.isBefore(pos1)) {
      return $5D.fromPositions(pos2, pos2);
    }
    return $5D.fromPositions(pos1, pos2);
  }
  /**
  ...
  extendToFullLines(range) {
    const start = $3(this.c, (x) => x <= range.start) ?? 0;
    const end = $5(this.c, (x) => range.endExclusive <= x) ?? this.b.length;
    return new $4E(start, end);
  }
};
...
function $tab(changes, originalLines, modifiedLines, hashedOriginalLines, hashedModifiedLines, timeout) {
...
function computeMovesFromSimpleDeletionsToSimpleInsertions(changes, originalLines, modifiedLines, timeout) {
  const moves = [];
  const deletions = changes.filter((c) => c.modified.isEmpty && c.original.length >= 3).map((d) => new $pab(d.original, originalLines, d));
  const insertions = new Set(changes.filter((c) => c.original.isEmpty && c.modified.length >= 3).map((d) => new $pab(d.modified, modifiedLines, d)));
  ...
  for (const deletion of deletions) {
    ...
    if (highestSimilarity > 0.9 && best) {
      ...
      moves.push(new $sN(deletion.range, best.range));
...
function computeUnchangedMoves(changes, hashedOriginalLines, hashedModifiedLines, originalLines, modifiedLines, timeout) {
  const moves = [];
  const original3LineHashes = new $Mc();
  for (const change of changes) {
    for (let i = change.original.startLineNumber; i < change.original.endLineNumberExclusive - 2; i++) {
      const key = `${hashedOriginalLines[i - 1]}:${hashedOriginalLines[i + 1 - 1]}:${hashedOriginalLines[i + 2 - 1]}`;
      original3LineHashes.add(key, { range: new $bF(i, i + 3) });
    }
  }
  const possibleMappings = [];
  changes.sort($9b((c) => c.modified.startLineNumber, $$b));
  for (const change of changes) {
    let lastMappings = [];
    for (let i = change.modified.startLineNumber; i < change.modified.endLineNumberExclusive - 2; i++) {
      const key = `${hashedModifiedLines[i - 1]}:${hashedModifiedLines[i + 1 - 1]}:${hashedModifiedLines[i + 2 - 1]}`;
      const currentModifiedRange = new $bF(i, i + 3);
      const nextMappings = [];
      original3LineHashes.forEach(key, ({ range }) => {
        for (const lastMapping of lastMappings) {
          if (lastMapping.originalLineRange.endLineNumberExclusive + 1 === range.endLineNumberExclusive && lastMapping.modifiedLineRange.endLineNumberExclusive + 1 === currentModifiedRange.endLineNumberExclusive) {
            lastMapping.originalLineRange = new $bF(lastMapping.originalLineRange.startLineNumber, range.endLineNumberExclusive);
            lastMapping.modifiedLineRange = new $bF(lastMapping.modifiedLineRange.startLineNumber, currentModifiedRange.endLineNumberExclusive);
  ...
  possibleMappings.sort($ac($9b((m) => m.modifiedLineRange.length, $$b)));
  const modifiedSet = new $cF();
  const originalSet = new $cF();
  for (const mapping of possibleMappings) {
    ...
    for (const s of modifiedIntersectedSections.ranges) {
      ...
      moves.push(new $sN(originalLineRange, modifiedLineRange));
      modifiedSet.addRange(modifiedLineRange);
      originalSet.addRange(originalLineRange);
    }
  }
  moves.sort($9b((m) => m.original.startLineNumber, $$b));
  const monotonousChanges = new $8(changes);
  for (let i = 0; i < moves.length; i++) {
    const move = moves[i];
    const firstTouchingChangeOrig = monotonousChanges.findLastMonotonous((c) => c.original.startLineNumber <= move.original.startLineNumber);
    const firstTouchingChangeMod = $3(changes, (c) => c.modified.startLineNumber <= move.modified.startLineNumber);
    const linesAbove = Math.max(move.original.startLineNumber - firstTouchingChangeOrig.original.startLineNumber, move.modified.startLineNumber - firstTouchingChangeMod.modified.startLineNumber);
    const lastTouchingChangeOrig = monotonousChanges.findLastMonotonous((c) => c.original.startLineNumber < move.original.endLineNumberExclusive);
    const lastTouchingChangeMod = $3(changes, (c) => c.modified.startLineNumber < move.modified.endLineNumberExclusive);
    ...
    if (extendToTop > 0) {
      originalSet.addRange(new $bF(move.original.startLineNumber - extendToTop, move.original.startLineNumber));
      modifiedSet.addRange(new $bF(move.modified.startLineNumber - extendToTop, move.modified.startLineNumber));
    }
    let extendToBottom;
    ...
    if (extendToBottom > 0) {
      originalSet.addRange(new $bF(move.original.endLineNumberExclusive, move.original.endLineNumberExclusive + extendToBottom));
      modifiedSet.addRange(new $bF(move.modified.endLineNumberExclusive, move.modified.endLineNumberExclusive + extendToBottom));
    }
    if (extendToTop > 0 || extendToBottom > 0) {
      moves[i] = new $sN(new $bF(move.original.startLineNumber - extendToTop, move.original.endLineNumberExclusive + extendToBottom), new $bF(move.modified.startLineNumber - extendToTop, move.modified.endLineNumberExclusive + extendToBottom));
    }
  }
  return moves;
}
function areLinesSimilar(line1, line2, timeout) {
  ...
  const myersDiffingAlgorithm = new $rab();
  const result = myersDiffingAlgorithm.compute(new $sab([line1], new $5D(1, 1, 1, line1.length), false), new $sab([line2], new $5D(1, 1, 1, line2.length), false), timeout);
  let commonNonSpaceCharCount = 0;
  const inverted = $jab.invert(result.diffs, line1.length);
  for (const seq of inverted) {
    seq.seq1Range.forEach((idx) => {
      if (!$oab(line1.charCodeAt(idx))) {
...
function removeMovesInSameDiff(changes, moves) {
  const changesMonotonous = new $8(changes);
  moves = moves.filter((m) => {
    const diffBeforeEndOfMoveOriginal = changesMonotonous.findLastMonotonous((c) => c.original.startLineNumber < m.original.endLineNumberExclusive) || new $sN(new $bF(1, 1), new $bF(1, 1));
    const diffBeforeEndOfMoveModified = $3(changes, (c) => c.modified.startLineNumber < m.modified.endLineNumberExclusive);
...
function $vab(sequence1, sequence2, sequenceDiffs) {
...
function joinSequenceDiffsByShifting(sequence1, sequence2, sequenceDiffs) {
  ...
  for (let i = 1; i < sequenceDiffs.length; i++) {
    ...
    if (cur.seq1Range.isEmpty || cur.seq2Range.isEmpty) {
      ...
      d--;
      if (d === length) {
        result[result.length - 1] = new $jab(new $4E(prevResult.seq1Range.start, cur.seq1Range.endExclusive - length), new $4E(prevResult.seq2Range.start, cur.seq2Range.endExclusive - length));
        continue;
      }
      cur = cur.delta(-d);
    }
  ...
  for (let i = 0; i < result.length - 1; i++) {
    ...
    if (cur.seq1Range.isEmpty || cur.seq2Range.isEmpty) {
      ...
      if (d === length) {
        result[i + 1] = new $jab(new $4E(cur.seq1Range.start + length, nextResult.seq1Range.endExclusive), new $4E(cur.seq2Range.start + length, nextResult.seq2Range.endExclusive));
        continue;
      }
      if (d > 0) {
        cur = cur.delta(d);
...
function shiftSequenceDiffs(sequence1, sequence2, sequenceDiffs) {
  ...
  for (let i = 0; i < sequenceDiffs.length; i++) {
    ...
    const seq1ValidRange = new $4E(prevDiff ? prevDiff.seq1Range.endExclusive + 1 : 0, nextDiff ? nextDiff.seq1Range.start - 1 : sequence1.length);
    const seq2ValidRange = new $4E(prevDiff ? prevDiff.seq2Range.endExclusive + 1 : 0, nextDiff ? nextDiff.seq2Range.start - 1 : sequence2.length);
...
function $wab(sequence1, sequence2, sequenceDiffs) {
  const result = [];
  for (const s of sequenceDiffs) {
    ...
    if (s.seq1Range.start - last.seq1Range.endExclusive <= 2 || s.seq2Range.start - last.seq2Range.endExclusive <= 2) {
      result[result.length - 1] = new $jab(last.seq1Range.join(s.seq1Range), last.seq2Range.join(s.seq2Range));
    } else {
      result.push(s);
    }
  }
...
function $xab(sequence1, sequence2, sequenceDiffs, findParent, force = false) {
  const equalMappings = $jab.invert(sequenceDiffs, sequence1.length);
  const additional = [];
  let lastPoint = new $kab(0, 0);
...
function $yab(sequence1, _sequence2, sequenceDiffs) {
  ...
  do {
    ...
    for (let i = 1; i < diffs.length; i++) {
      let shouldJoinDiffs2 = function(before, after) {
        const unchangedRange = new $4E(lastResult.seq1Range.endExclusive, cur.seq1Range.start);
...
function $zab(sequence1, sequence2, sequenceDiffs) {
  let diffs = sequenceDiffs;
  if (diffs.length === 0) {
    return diffs;
  }
  ...
  const newDiffs = [];
  $Gb(diffs, (prev, cur, next) => {
    let newDiff = cur;
    function shouldMarkAsChanged(text) {
      return text.length > 0 && text.trim().length <= 3 && cur.seq1Range.length + cur.seq2Range.length > 100;
    }
    const fullRange1 = sequence1.extendToFullLines(cur.seq1Range);
    const prefix = sequence1.getText(new $4E(fullRange1.start, cur.seq1Range.start));
    if (shouldMarkAsChanged(prefix)) {
      newDiff = newDiff.deltaStart(-prefix.length);
    }
    const suffix = sequence1.getText(new $4E(cur.seq1Range.endExclusive, fullRange1.endExclusive));
    if (shouldMarkAsChanged(suffix)) {
      newDiff = newDiff.deltaEnd(suffix.length);
    }
    const availableSpace = $jab.fromOffsetPairs(prev ? prev.getEndExclusives() : $kab.zero, next ? next.getStarts() : $kab.max);
...
var $Aab = class {
  ...
  computeDiff(originalLines, modifiedLines, options) {
    if (originalLines.length <= 1 && $yb(originalLines, modifiedLines, (a, b) => a === b)) {
      return new $yN([], [], false);
    }
    if (originalLines.length === 1 && originalLines[0].length === 0 || modifiedLines.length === 1 && modifiedLines[0].length === 0) {
      return new $yN([
        new $tN(new $bF(1, originalLines.length + 1), new $bF(1, modifiedLines.length + 1), [
          new $uN(new $5D(1, 1, originalLines.length, originalLines[originalLines.length - 1].length + 1), new $5D(1, 1, modifiedLines.length, modifiedLines[modifiedLines.length - 1].length + 1))
        ])
      ], [], false);
    }
    const timeout = options.maxComputationTimeMs === 0 ? $lab.instance : new $mab(options.maxComputationTimeMs);
    ...
    const sequence1 = new $uab(originalLinesHashes, originalLines);
    const sequence2 = new $uab(modifiedLinesHashes, modifiedLines);
    ...
    lineAlignments = $vab(sequence1, sequence2, lineAlignments);
    lineAlignments = $yab(sequence1, sequence2, lineAlignments);
    const alignments = [];
    const scanForWhitespaceChanges = (equalLinesCount) => {
      ...
      for (let i = 0; i < equalLinesCount; i++) {
        ...
        if (originalLines[seq1Offset] !== modifiedLines[seq2Offset]) {
          const characterDiffs = this.h(originalLines, modifiedLines, new $jab(new $4E(seq1Offset, seq1Offset + 1), new $4E(seq2Offset, seq2Offset + 1)), timeout, considerWhitespaceChanges, options);
    ...
    for (const diff of lineAlignments) {
      $Uc(() => diff.seq1Range.start - seq1LastStart === diff.seq2Range.start - seq2LastStart);
    ...
    const changes = $vN(alignments, new $qN(originalLines), new $qN(modifiedLines));
    let moves = [];
    ...
    return new $yN(changes, moves, hitTimeout);
  }
  g(changes, originalLines, modifiedLines, hashedOriginalLines, hashedModifiedLines, timeout, considerWhitespaceChanges, options) {
    const moves = $tab(changes, originalLines, modifiedLines, hashedOriginalLines, hashedModifiedLines, timeout);
    const movesWithDiffs = moves.map((m) => {
      const moveChanges = this.h(originalLines, modifiedLines, new $jab(m.original.toOffsetRange(), m.modified.toOffsetRange()), timeout, considerWhitespaceChanges, options);
      const mappings = $vN(moveChanges.mappings, new $qN(originalLines), new $qN(modifiedLines), true);
      return new $zN(m, mappings);
    });
  ...
  h(originalLines, modifiedLines, diff, timeout, considerWhitespaceChanges, options) {
    ...
    const slice1 = new $sab(originalLines, rangeMapping.originalRange, considerWhitespaceChanges);
    const slice2 = new $sab(modifiedLines, rangeMapping.modifiedRange, considerWhitespaceChanges);
    ...
    if (check) {
      $jab.assertSorted(diffs);
    }
    diffs = $vab(slice1, slice2, diffs);
    if (check) {
      $jab.assertSorted(diffs);
    }
    diffs = $xab(slice1, slice2, diffs, (seq, idx) => seq.findWordContaining(idx));
    if (check) {
      $jab.assertSorted(diffs);
    }
    if (options.extendToSubwords) {
      diffs = $xab(slice1, slice2, diffs, (seq, idx) => seq.findSubWordContaining(idx), true);
      if (check) {
        $jab.assertSorted(diffs);
      }
    }
    diffs = $wab(slice1, slice2, diffs);
    if (check) {
      $jab.assertSorted(diffs);
    }
    diffs = $zab(slice1, slice2, diffs);
    if (check) {
      $jab.assertSorted(diffs);
    }
    const result = diffs.map((d) => new $uN(slice1.translateRange(d.seq1Range), slice2.translateRange(d.seq2Range)));
    if (check) {
      $uN.assertSorted(result);
    }
    return {
      mappings: result,
...
function toLineRangeMapping(sequenceDiff) {
  return new $sN(new $bF(sequenceDiff.seq1Range.start + 1, sequenceDiff.seq1Range.endExclusive + 1), new $bF(sequenceDiff.seq2Range.start + 1, sequenceDiff.seq2Range.endExclusive + 1));
}
...
var $qo = class _$qo {
  ...
  static toRGBA(hsla) {
    ...
    return new $po(Math.round(r * 255), Math.round(g * 255), Math.round(b * 255), a);
  }
};
var $ro = class _$ro {
  constructor(h, s, v, a) {
  ...
  static fromRGBA(rgba) {
    ...
    return new _$ro(Math.round(m * 60), s, cmax, rgba.a);
  }
  ...
  static toRGBA(hsva) {
    ...
    return new $po(r, g, b, a);
  }
};
var $so = class _$so {
  static fromHex(hex) {
    return _$so.Format.CSS.parseHex(hex) || _$so.red;
  }
  static equals(a, b) {
    if (!a && !b) {
      return true;
  ...
  get hsva() {
    ...
    return $ro.fromRGBA(this.rgba);
  }
  constructor(arg) {
    if (!arg) {
      throw new Error("Color needs a value");
    } else if (arg instanceof $po) {
      this.rgba = arg;
    } else if (arg instanceof $qo) {
      this.i = arg;
      this.rgba = $qo.toRGBA(arg);
    } else if (arg instanceof $ro) {
      this.j = arg;
      this.rgba = $ro.toRGBA(arg);
    } else {
      throw new Error("Invalid color ctor argument");
    }
  }
  equals(other) {
    return !!other && $po.equals(this.rgba, other.rgba) && $qo.equals(this.hsla, other.hsla) && $ro.equals(this.hsva, other.hsva);
  }
  /**
  ...
  getRelativeLuminance() {
    const R = _$so.k(this.rgba.r);
    const G = _$so.k(this.rgba.g);
    const B = _$so.k(this.rgba.b);
  ...
  reduceRelativeLuminace(foreground, ratio) {
    ...
    while (cr < ratio && (fgR > 0 || fgG > 0 || fgB > 0)) {
      ...
      cr = this.getContrastRatio(new _$so(new $po(fgR, fgG, fgB)));
    }
    return new _$so(new $po(fgR, fgG, fgB));
  }
  /**
  ...
  lighten(factor) {
    return new _$so(new $qo(this.hsla.h, this.hsla.s, this.hsla.l + this.hsla.l * factor, this.hsla.a));
  }
  darken(factor) {
    return new _$so(new $qo(this.hsla.h, this.hsla.s, this.hsla.l - this.hsla.l * factor, this.hsla.a));
  }
  transparent(factor) {
    ...
    return new _$so(new $po(r, g, b, a * factor));
  }
  isTransparent() {
  ...
  opposite() {
    return new _$so(new $po(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a));
  }
  blend(c) {
    const rgba = c.rgba;
    ...
    if (a < 1e-6) {
      return _$so.transparent;
    }
    const r = this.rgba.r * thisA / a + rgba.r * colorA * (1 - thisA) / a;
    const g = this.rgba.g * thisA / a + rgba.g * colorA * (1 - thisA) / a;
    const b = this.rgba.b * thisA / a + rgba.b * colorA * (1 - thisA) / a;
    return new _$so(new $po(r, g, b, a));
  }
  makeOpaque(opaqueBackground) {
    ...
    const { r, g, b, a } = this.rgba;
    return new _$so(new $po(opaqueBackground.rgba.r - a * (opaqueBackground.rgba.r - r), opaqueBackground.rgba.g - a * (opaqueBackground.rgba.g - g), opaqueBackground.rgba.b - a * (opaqueBackground.rgba.b - b), 1));
  }
  flatten(...backgrounds) {
    const background = backgrounds.reduceRight((accumulator, color) => {
      return _$so.o(color, accumulator);
    });
    return _$so.o(this, background);
  }
  static o(foreground, background) {
    const backgroundAlpha = 1 - foreground.rgba.a;
    return new _$so(new $po(backgroundAlpha * background.rgba.r + foreground.rgba.a * foreground.rgba.r, backgroundAlpha * background.rgba.g + foreground.rgba.a * foreground.rgba.g, backgroundAlpha * background.rgba.b + foreground.rgba.a * foreground.rgba.b));
  }
  toString() {
    if (!this.u) {
      this.u = _$so.Format.CSS.format(this);
    }
    return this.u;
  }
  toNumber32Bit() {
  ...
  static {
    this.white = new _$so(new $po(255, 255, 255, 1));
  }
  static {
    this.black = new _$so(new $po(0, 0, 0, 1));
  }
  static {
    this.red = new _$so(new $po(255, 0, 0, 1));
  }
  static {
    this.blue = new _$so(new $po(0, 0, 255, 1));
  }
  static {
    this.green = new _$so(new $po(0, 255, 0, 1));
  }
  static {
    this.cyan = new _$so(new $po(0, 255, 255, 1));
  }
  static {
    this.lightgrey = new _$so(new $po(211, 211, 211, 1));
  }
  static {
    this.transparent = new _$so(new $po(0, 0, 0, 0));
  }
};
(function($so2) {
  let Format;
  (function(Format2) {
    ...
    (function(CSS2) {
      function formatRGB(color) {
        ...
        return $so2.Format.CSS.formatRGBA(color);
      }
      ...
      function formatHSL(color) {
        ...
        return $so2.Format.CSS.formatHSLA(color);
      }
      ...
      function formatHexA(color, compact = false) {
        if (compact && color.rgba.a === 1) {
          return $so2.Format.CSS.formatHex(color);
        }
      ...
      function parseNamedKeyword(css) {
        switch (css) {
          case "aliceblue":
            return new $so2(new $po(240, 248, 255, 1));
          case "antiquewhite":
            return new $so2(new $po(250, 235, 215, 1));
          case "aqua":
            return new $so2(new $po(0, 255, 255, 1));
          case "aquamarine":
            return new $so2(new $po(127, 255, 212, 1));
          case "azure":
            return new $so2(new $po(240, 255, 255, 1));
          case "beige":
            return new $so2(new $po(245, 245, 220, 1));
          case "bisque":
            return new $so2(new $po(255, 228, 196, 1));
          case "black":
            return new $so2(new $po(0, 0, 0, 1));
          case "blanchedalmond":
            return new $so2(new $po(255, 235, 205, 1));
          case "blue":
            return new $so2(new $po(0, 0, 255, 1));
          case "blueviolet":
            return new $so2(new $po(138, 43, 226, 1));
          case "brown":
            return new $so2(new $po(165, 42, 42, 1));
          case "burlywood":
            return new $so2(new $po(222, 184, 135, 1));
          case "cadetblue":
            return new $so2(new $po(95, 158, 160, 1));
          case "chartreuse":
            return new $so2(new $po(127, 255, 0, 1));
          case "chocolate":
            return new $so2(new $po(210, 105, 30, 1));
          case "coral":
            return new $so2(new $po(255, 127, 80, 1));
          case "cornflowerblue":
            return new $so2(new $po(100, 149, 237, 1));
          case "cornsilk":
            return new $so2(new $po(255, 248, 220, 1));
          case "crimson":
            return new $so2(new $po(220, 20, 60, 1));
          case "cyan":
            return new $so2(new $po(0, 255, 255, 1));
          case "darkblue":
            return new $so2(new $po(0, 0, 139, 1));
          case "darkcyan":
            return new $so2(new $po(0, 139, 139, 1));
          case "darkgoldenrod":
            return new $so2(new $po(184, 134, 11, 1));
          case "darkgray":
            return new $so2(new $po(169, 169, 169, 1));
          case "darkgreen":
            return new $so2(new $po(0, 100, 0, 1));
          case "darkgrey":
            return new $so2(new $po(169, 169, 169, 1));
          case "darkkhaki":
            return new $so2(new $po(189, 183, 107, 1));
          case "darkmagenta":
            return new $so2(new $po(139, 0, 139, 1));
          case "darkolivegreen":
            return new $so2(new $po(85, 107, 47, 1));
          case "darkorange":
            return new $so2(new $po(255, 140, 0, 1));
          case "darkorchid":
            return new $so2(new $po(153, 50, 204, 1));
          case "darkred":
            return new $so2(new $po(139, 0, 0, 1));
          case "darksalmon":
            return new $so2(new $po(233, 150, 122, 1));
          case "darkseagreen":
            return new $so2(new $po(143, 188, 143, 1));
          case "darkslateblue":
            return new $so2(new $po(72, 61, 139, 1));
          case "darkslategray":
            return new $so2(new $po(47, 79, 79, 1));
          case "darkslategrey":
            return new $so2(new $po(47, 79, 79, 1));
          case "darkturquoise":
            return new $so2(new $po(0, 206, 209, 1));
          case "darkviolet":
            return new $so2(new $po(148, 0, 211, 1));
          case "deeppink":
            return new $so2(new $po(255, 20, 147, 1));
          case "deepskyblue":
            return new $so2(new $po(0, 191, 255, 1));
          case "dimgray":
            return new $so2(new $po(105, 105, 105, 1));
          case "dimgrey":
            return new $so2(new $po(105, 105, 105, 1));
          case "dodgerblue":
            return new $so2(new $po(30, 144, 255, 1));
          case "firebrick":
            return new $so2(new $po(178, 34, 34, 1));
          case "floralwhite":
            return new $so2(new $po(255, 250, 240, 1));
          case "forestgreen":
            return new $so2(new $po(34, 139, 34, 1));
          case "fuchsia":
            return new $so2(new $po(255, 0, 255, 1));
          case "gainsboro":
            return new $so2(new $po(220, 220, 220, 1));
          case "ghostwhite":
            return new $so2(new $po(248, 248, 255, 1));
          case "gold":
            return new $so2(new $po(255, 215, 0, 1));
          case "goldenrod":
            return new $so2(new $po(218, 165, 32, 1));
          case "gray":
            return new $so2(new $po(128, 128, 128, 1));
          case "green":
            return new $so2(new $po(0, 128, 0, 1));
          case "greenyellow":
            return new $so2(new $po(173, 255, 47, 1));
          case "grey":
            return new $so2(new $po(128, 128, 128, 1));
          case "honeydew":
            return new $so2(new $po(240, 255, 240, 1));
          case "hotpink":
            return new $so2(new $po(255, 105, 180, 1));
          case "indianred":
            return new $so2(new $po(205, 92, 92, 1));
          case "indigo":
            return new $so2(new $po(75, 0, 130, 1));
          case "ivory":
            return new $so2(new $po(255, 255, 240, 1));
          case "khaki":
            return new $so2(new $po(240, 230, 140, 1));
          case "lavender":
            return new $so2(new $po(230, 230, 250, 1));
          case "lavenderblush":
            return new $so2(new $po(255, 240, 245, 1));
          case "lawngreen":
            return new $so2(new $po(124, 252, 0, 1));
          case "lemonchiffon":
            return new $so2(new $po(255, 250, 205, 1));
          case "lightblue":
            return new $so2(new $po(173, 216, 230, 1));
          case "lightcoral":
            return new $so2(new $po(240, 128, 128, 1));
          case "lightcyan":
            return new $so2(new $po(224, 255, 255, 1));
          case "lightgoldenrodyellow":
            return new $so2(new $po(250, 250, 210, 1));
          case "lightgray":
            return new $so2(new $po(211, 211, 211, 1));
          case "lightgreen":
            return new $so2(new $po(144, 238, 144, 1));
          case "lightgrey":
            return new $so2(new $po(211, 211, 211, 1));
          case "lightpink":
            return new $so2(new $po(255, 182, 193, 1));
          case "lightsalmon":
            return new $so2(new $po(255, 160, 122, 1));
          case "lightseagreen":
            return new $so2(new $po(32, 178, 170, 1));
          case "lightskyblue":
            return new $so2(new $po(135, 206, 250, 1));
          case "lightslategray":
            return new $so2(new $po(119, 136, 153, 1));
          case "lightslategrey":
            return new $so2(new $po(119, 136, 153, 1));
          case "lightsteelblue":
            return new $so2(new $po(176, 196, 222, 1));
          case "lightyellow":
            return new $so2(new $po(255, 255, 224, 1));
          case "lime":
            return new $so2(new $po(0, 255, 0, 1));
          case "limegreen":
            return new $so2(new $po(50, 205, 50, 1));
          case "linen":
            return new $so2(new $po(250, 240, 230, 1));
          case "magenta":
            return new $so2(new $po(255, 0, 255, 1));
          case "maroon":
            return new $so2(new $po(128, 0, 0, 1));
          case "mediumaquamarine":
            return new $so2(new $po(102, 205, 170, 1));
          case "mediumblue":
            return new $so2(new $po(0, 0, 205, 1));
          case "mediumorchid":
            return new $so2(new $po(186, 85, 211, 1));
          case "mediumpurple":
            return new $so2(new $po(147, 112, 219, 1));
          case "mediumseagreen":
            return new $so2(new $po(60, 179, 113, 1));
          case "mediumslateblue":
            return new $so2(new $po(123, 104, 238, 1));
          case "mediumspringgreen":
            return new $so2(new $po(0, 250, 154, 1));
          case "mediumturquoise":
            return new $so2(new $po(72, 209, 204, 1));
          case "mediumvioletred":
            return new $so2(new $po(199, 21, 133, 1));
          case "midnightblue":
            return new $so2(new $po(25, 25, 112, 1));
          case "mintcream":
            return new $so2(new $po(245, 255, 250, 1));
          case "mistyrose":
            return new $so2(new $po(255, 228, 225, 1));
          case "moccasin":
            return new $so2(new $po(255, 228, 181, 1));
          case "navajowhite":
            return new $so2(new $po(255, 222, 173, 1));
          case "navy":
            return new $so2(new $po(0, 0, 128, 1));
          case "oldlace":
            return new $so2(new $po(253, 245, 230, 1));
          case "olive":
            return new $so2(new $po(128, 128, 0, 1));
          case "olivedrab":
            return new $so2(new $po(107, 142, 35, 1));
          case "orange":
            return new $so2(new $po(255, 165, 0, 1));
          case "orangered":
            return new $so2(new $po(255, 69, 0, 1));
          case "orchid":
            return new $so2(new $po(218, 112, 214, 1));
          case "palegoldenrod":
            return new $so2(new $po(238, 232, 170, 1));
          case "palegreen":
            return new $so2(new $po(152, 251, 152, 1));
          case "paleturquoise":
            return new $so2(new $po(175, 238, 238, 1));
          case "palevioletred":
            return new $so2(new $po(219, 112, 147, 1));
          case "papayawhip":
            return new $so2(new $po(255, 239, 213, 1));
          case "peachpuff":
            return new $so2(new $po(255, 218, 185, 1));
          case "peru":
            return new $so2(new $po(205, 133, 63, 1));
          case "pink":
            return new $so2(new $po(255, 192, 203, 1));
          case "plum":
            return new $so2(new $po(221, 160, 221, 1));
          case "powderblue":
            return new $so2(new $po(176, 224, 230, 1));
          case "purple":
            return new $so2(new $po(128, 0, 128, 1));
          case "rebeccapurple":
            return new $so2(new $po(102, 51, 153, 1));
          case "red":
            return new $so2(new $po(255, 0, 0, 1));
          case "rosybrown":
            return new $so2(new $po(188, 143, 143, 1));
          case "royalblue":
            return new $so2(new $po(65, 105, 225, 1));
          case "saddlebrown":
            return new $so2(new $po(139, 69, 19, 1));
          case "salmon":
            return new $so2(new $po(250, 128, 114, 1));
          case "sandybrown":
            return new $so2(new $po(244, 164, 96, 1));
          case "seagreen":
            return new $so2(new $po(46, 139, 87, 1));
          case "seashell":
            return new $so2(new $po(255, 245, 238, 1));
          case "sienna":
            return new $so2(new $po(160, 82, 45, 1));
          case "silver":
            return new $so2(new $po(192, 192, 192, 1));
          case "skyblue":
            return new $so2(new $po(135, 206, 235, 1));
          case "slateblue":
            return new $so2(new $po(106, 90, 205, 1));
          case "slategray":
            return new $so2(new $po(112, 128, 144, 1));
          case "slategrey":
            return new $so2(new $po(112, 128, 144, 1));
          case "snow":
            return new $so2(new $po(255, 250, 250, 1));
          case "springgreen":
            return new $so2(new $po(0, 255, 127, 1));
          case "steelblue":
            return new $so2(new $po(70, 130, 180, 1));
          case "tan":
            return new $so2(new $po(210, 180, 140, 1));
          case "teal":
            return new $so2(new $po(0, 128, 128, 1));
          case "thistle":
            return new $so2(new $po(216, 191, 216, 1));
          case "tomato":
            return new $so2(new $po(255, 99, 71, 1));
          case "turquoise":
            return new $so2(new $po(64, 224, 208, 1));
          case "violet":
            return new $so2(new $po(238, 130, 238, 1));
          case "wheat":
            return new $so2(new $po(245, 222, 179, 1));
          case "white":
            return new $so2(new $po(255, 255, 255, 1));
          case "whitesmoke":
            return new $so2(new $po(245, 245, 245, 1));
          case "yellow":
            return new $so2(new $po(255, 255, 0, 1));
          case "yellowgreen":
            return new $so2(new $po(154, 205, 50, 1));
          default:
            return null;
        }
      }
      function parseHex(hex) {
        ...
        if (length === 9) {
          ...
          return new $so2(new $po(r, g, b, a / 255));
        }
        if (length === 4) {
          const r = _parseHexDigit(hex.charCodeAt(1));
          const g = _parseHexDigit(hex.charCodeAt(2));
          const b = _parseHexDigit(hex.charCodeAt(3));
          return new $so2(new $po(16 * r + r, 16 * g + g, 16 * b + b));
        }
        if (length === 5) {
          const r = _parseHexDigit(hex.charCodeAt(1));
          const g = _parseHexDigit(hex.charCodeAt(2));
          ...
          return new $so2(new $po(16 * r + r, 16 * g + g, 16 * b + b, (16 * a + a) / 255));
        }
        return null;
      }
...
function _findHexColorInformation(range, hexValue) {
  ...
  const parsedHexColor = $so.Format.CSS.parseHex(hexValue);
  if (!parsedHexColor) {
    return;
  }
  return {
...
function _findHSLColorInformation(range, matches, isAlpha) {
  ...
  const captureGroups = match.values();
  const parsedRegex = _parseCaptureGroups(captureGroups);
  const colorEquivalent = new $so(new $qo(parsedRegex[0], parsedRegex[1] / 100, parsedRegex[2] / 100, isAlpha ? parsedRegex[3] : 1));
  return {
    range,
...
function $Dab(model, options) {
  ...
  if (options.findMarkSectionHeaders) {
    const markHeaders = $Eab(model, options);
...
function $Eab(model, options) {
  const markHeaders = [];
  const endLineNumber = model.getLineCount();
  const multiline = $3I(options.markSectionHeaderRegex);
...
var RemoteAuthoritiesImpl = class {
  ...
  setServerRootPath(product, serverBasePath) {
    this.f = $lc.join(serverBasePath ?? "/", $uh(product));
  }
  getServerRootPath() {
    return this.f;
  }
...
var $Fab = 60 * 1e3;
var $Gab = "workerTextModelSync";
var $Iab = class {
  constructor() {
    this.a = /* @__PURE__ */ Object.create(null);
  }
  bindToServer(workerServer) {
    workerServer.setChannel($Gab, this);
  }
  getModel(uri) {
    return this.a[uri];
  }
  ...
  $acceptNewModel(data) {
    this.a[data.url] = new $Jab(URI.parse(data.url), data.lines, data.EOL, data.versionId);
  }
...
var $Jab = class extends $FN {
  ...
  findMatches(regex) {
    ...
    for (let i = 0; i < this.b.length; i++) {
      const line = this.b[i];
      const offsetToAdd = this.offsetAt(new $4D(i + 1, 1));
  ...
  getWordAtPosition(position, wordDefinition) {
    const wordAtText = $iE(position.column, $gE(wordDefinition), this.b[position.lineNumber - 1], 0);
    if (wordAtText) {
      return new $5D(position.lineNumber, wordAtText.startColumn, position.lineNumber, wordAtText.endColumn);
    }
    return null;
  }
...
var $Kab = class _$Kab {
  ...
  async $computeUnicodeHighlights(url, options, range) {
    ...
    return $bab.computeUnicodeHighlights(model, options, range);
  }
  ...
  async $computeDiff(originalUrl, modifiedUrl, options, algorithm) {
    ...
    const result = _$Kab.h(original, modified, options, algorithm);
    return result;
  }
  static h(originalTextModel, modifiedTextModel, options, algorithm) {
    const diffAlgorithm = algorithm === "advanced" ? $Bab.getDefault() : $Bab.getLegacy();
  ...
  async $computeMoreMinimalEdits(modelUrl, edits, pretty) {
    ...
    edits = edits.slice(0).sort((a, b) => {
      if (a.range && b.range) {
        return $5D.compareRangesUsingStarts(a.range, b.range);
      }
    ...
    for (let readIndex = 1; readIndex < edits.length; readIndex++) {
      if ($5D.getEndPosition(edits[writeIndex].range).equals($5D.getStartPosition(edits[readIndex].range))) {
        edits[writeIndex].range = $5D.fromPositions($5D.getStartPosition(edits[writeIndex].range), $5D.getEndPosition(edits[readIndex].range));
    ...
    for (let { range, text, eol } of edits) {
      ...
      if (Math.max(text.length, original.length) > _$Kab.k) {
        result.push({ range, text });
        continue;
      }
      const changes = $1D(original, text, pretty);
      const editOffset = model.offsetAt($5D.lift(range).getStartPosition());
      for (const change of changes) {
  ...
  $computeHumanReadableDiff(modelUrl, edits, options) {
    ...
    for (let { range, text, eol } of edits) {
      let addPositions2 = function(pos1, pos2) {
        return new $4D(pos1.lineNumber + pos2.lineNumber - 1, pos2.lineNumber === 1 ? pos1.column + pos2.column - 1 : pos2.column);
      ...
      const diff = $Bab.getDefault().computeDiff(originalLines, modifiedLines, options);
      const start = $5D.lift(range).getStartPosition();
      for (const c of diff.changes) {
        if (c.innerChanges) {
          for (const x of c.innerChanges) {
            result.push({
              range: $5D.fromPositions(addPositions2(start, x.originalRange.getStartPosition()), addPositions2(start, x.originalRange.getEndPosition())),
              text: getText2(modifiedLines, x.modifiedRange).join(model.eol)
            });
          }
        } else {
          throw new $vb("The experimental diff algorithm always produces inner changes");
        }
      }
    }
    if (typeof lastEol === "number") {
  ...
  async $navigateValueSet(modelUrl, range, up, wordDef, wordDefFlags) {
    ...
    const result = $fab.INSTANCE.navigateValueSet(range, selectionText, wordRange, word, up);
    return result;
  }
